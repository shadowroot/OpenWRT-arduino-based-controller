...
int serial_setup(int fd,int speed,int parity){
	...
	SERIAL_BUFFER = (char*)malloc(SERIAL_BUFFER_SIZE);
	 struct termios tty;
...
/**
 * Find if there is a better way than pooling to get serial data.
 * @return
 */
int startSerialPooling(){
	char * buffer = NULL;
	int size = 0;
	if(SERIAL_STATUS == SERIAL_STATUS_ERROR){
		return 1;
	}
	while(!LIB_SERIAL_STATUS){
		if(SERIAL_STATUS == SERIAL_STATUS_ERROR){
			error_message("error serial %d serial collapsed");
		}
		if(SERIAL_BUFFER_READY_TO_RECEIVE){
			serial_read();
			SERIAL_BUFFER_READY_TO_RECEIVE = FALSE;
			SERIAL_BUFFER_RECEIVED = TRUE;
		}
		else if(SERIAL_BUFFER_RECEIVED){
			size = __serial_to_json(&buffer);
			SERIAL_BUFFER_RECEIVED = FALSE;
		}
		else if(SERIAL_BUFFER_READY_TO_WRITE){
			serial_write();
			SERIAL_BUFFER_READY_TO_WRITE = FALSE;
		}
	}
...
int init_serial(){
	...
	if(!pid){
		...
		SERIAL_STATUS = SERIAL_STATUS_READY;
		if(startSerialPooling()){
			error_message("error %d serial pooling failed");
		}
 ...
 * @return
 */
int __json_parse_serial(const char * data){
	int i=0,objects=0;
	char inside_string = FALSE;

	for(;data[i];i++){
		if(data[i] == '\"'){
			inside_string ? FALSE : TRUE;
		}
		if(!inside_string){
			if(data[i] == '{'){
				objects++;
				...
				SERIAL_BUFFER[SERIAL_CURRENT_BUFFER_SIZE++] = ' ';
			}
			else if(data[i] == ','){
				SERIAL_BUFFER[SERIAL_CURRENT_BUFFER_SIZE++] = '\n';
			}
			...
			SERIAL_BUFFER[SERIAL_CURRENT_BUFFER_SIZE++] = data[i];
		}
	}
