...
STRING loadString(char * str_in){
	...
	/*
	 * Control if is concatenated or not
	 */
	if(*tmp == '"'){
		*tmp = 0;
	...
	*tmp = 0;//NULL termination makes string
	str.string = str_in;
	str.length = size;
	str.references = -1;
	return str;
	...
	while(str[pos]){
		...
		switch(str[pos]){
			case '{':
				...
				tmp_new_token->value.object = NULL;
 ...
 * @param string
 */
void __free_string(STRING * string){
 ...
 * @param objects
 */
void __free_object(struct TObject * objects){
	struct TToken * tmp = (struct TToken *)objects->tokens;
	while(tmp){
		free(tmp->param);
		switch(tmp->type){
		case TYPE_OBJECT:
			free(&(tmp->value.object));
			break;
		case TYPE_BOOL:
			break;
		case TYPE_STRING:
